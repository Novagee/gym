...
-(IBAction)gotoGenderScreen:(id)sender {
    ...
    NSUInteger numberOfMatches = [regex numberOfMatchesInString:_txtUserName.text options:NSMatchingWithTransparentBounds range:NSMakeRange(0, _txtUserName.text.length)];
    
    if (numberOfMatches > 0) {
        [appDelegate.window makeToast:@"非法字符，请重新输入" backgroundColor:RedColor];
        return;
    }
