...
-(IBAction)onClickUnfriend:(id)sender {
    ...
    IBActionSheet *standardIBAS = [[IBActionSheet alloc] initWithTitle:@"确定从朋友列表删除？"  cancelButtonTitle:@"取消" destructiveButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
-(IBAction)inviteFriends:(id)sender {
    ...
    IBActionSheet *standardIBAS = [[IBActionSheet alloc] initWithTitle:@"从电话簿里邀请好友加入一起来玩哦！"  cancelButtonTitle:@"取消" destructiveButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
-(void)fetchFriendsList {
    ...
    [client GET:[NSString stringWithFormat:@"%@r/friend/?",API_HOME] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (![[NSUserDefaults standardUserDefaults] boolForKey:TCHTutorialForFriends]) {
            [self showTutorial];
            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:TCHTutorialForFriends];
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        cell.weChatButton.hidden = NO;
        CGRect frame = cell.unfriendButton.frame;
        CGRect frame_wechat = cell.weChatButton.frame;
        frame_wechat.origin.y = frame.origin.y+10;
        cell.weChatButton.frame = frame_wechat;
        ...
        cell.time.text = [appDelegate dateTimeDifference:[dict objectForKey:@"createtimeForUI"]];
    }
...
-(void)sendSMS:(NSArray *)recipients {
    ...
    if([MFMessageComposeViewController canSendText]) {
        ...
        NSString *message = @"我在用“简约”约TA，太酷了，你也试试吧！加入从速哦!";
        ...
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"错误" message:@"您的设备无法发送短信" delegate:nil cancelButtonTitle:@"行" otherButtonTitles:nil];
        [alert show];
    }
}
