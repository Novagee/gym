...
-(void)showTutorial {
    ...
    if (IS_HEIGHT_GTE_568) {
        [view loadImageWithImageName:[NSString stringWithFormat:@"scroll-3-iPhone5.png"]];
    } else {
        [view loadImageWithImageName:[NSString stringWithFormat:@"scroll-3-iPhone4.png"]];
    }
...
-(IBAction)onClickClose:(id)sender {
    ...
    IBActionSheet *standardIBAS = [[IBActionSheet alloc] initWithTitle:@"确定要删除此条信息？"  cancelButtonTitle:@"取消" destructiveButtonTitle:@"删除" otherButtonTitles:nil, nil];
...
-(IBAction)onClickBlockButton:(id)sender {
        IBActionSheet *blockIBAS = [[IBActionSheet alloc] initWithTitle:@"确定要阻止此人所有信息？"  cancelButtonTitle:@"取消" destructiveButtonTitle:@"阻止" otherButtonTitles:nil, nil];
        blockIBAS.callbackBlock =
        ^(IBActionSheetCallbackType result, NSInteger buttonIndex) {
            UIButton *button = (UIButton *)sender;
            NSDictionary *dict = [_arrPeople objectAtIndex:button.tag-1];
            
            [appDelegate showLoading];
            [self blockUserMessages:[dict objectForKey:@"id"]];
        };
        [blockIBAS showInView:self];
}
...
-(void)fetchChatHistory {
    ...
    [client GET:[NSString stringWithFormat:@"%@r/message/?",API_HOME] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (![[NSUserDefaults standardUserDefaults] boolForKey:TCHTutorialForMessage]) {
            [self showTutorial];
            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:TCHTutorialForMessage];
        }
...
-(void)sendRequest:(NSString *)receiver {
    ...
    [client POST:[NSString stringWithFormat:@"%@r/message/sendFriendRequest?",API_HOME] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[[responseObject objectForKey:@"ack"] lowercaseString] isEqualToString:@"success"]) {
            ...
            [appDelegate.window makeToast:@"您已发过请求，请等待对方回复" backgroundColor:[UIColor redColor]];
        } else if([[responseObject objectForKey:@"error"] isEqualToString:@"You are friends already"]){
            [appDelegate.window makeToast:@"你们已经是朋友了" backgroundColor:[UIColor redColor]];
            ...
            [appDelegate.window makeToast:@"对方已屏蔽您的请求" backgroundColor:[UIColor redColor]];
...
-(void)acceptRequestWithMessageId:(NSString *)messageId {
    ...
    [client POST:[NSString stringWithFormat:@"%@r/message/acceptFriendRequest?",API_HOME] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[[responseObject objectForKey:@"ack"] lowercaseString] isEqualToString:@"success"]) {
            ...
            [appDelegate.window makeToast:@"您已经发过请求了，请等待对方回复" backgroundColor:[UIColor redColor]];
        } else if([[responseObject objectForKey:@"error"] isEqualToString:@"You are friends already"]){
            [appDelegate.window makeToast:@"你们已经是朋友了" backgroundColor:[UIColor redColor]];
        } else {
            [appDelegate.window makeToast:ServerDBError backgroundColor:[UIColor redColor]];
...
-(void)blockUserMessages:(NSString *)messageId {
    ...
    [client POST:[NSString stringWithFormat:@"%@r/message/block?",API_HOME] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        
        if ([[[responseObject objectForKey:@"ack"] lowercaseString] isEqualToString:@"success"]) {
            [appDelegate.window makeToast:UserMessageBlockedSuccessfully backgroundColor:GreenColor];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        [cell.blockButton addTarget:self action:@selector(onClickBlockButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        cell.time.text = [appDelegate dateTimeDifference:[dict objectForKey:@"sendtimeForUI"]];
    }
